...
public class TextTool: NSObject, DrawingTool {
	...
	public private(set) var selectedShape: TextShape?
	...
	public func applyRemoveShapeOperation(context: ToolOperationContext) {
		guard let shape = selectedShape else { return }
...
extension TextTool: UITextViewDelegate {
	public func textViewDidChange(_ textView: UITextView) {
		guard let shape = selectedShape else { return }
		delegate?.textToolWillUpdate(shape: shape)
		shape.text = textView.text ?? ""
		updateShapeFrame()
		delegate?.textToolDidUpdate(shape: shape)
