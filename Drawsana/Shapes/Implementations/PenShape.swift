...
public struct PenLineSegment: Codable {
	public private(set) var a: CGPoint
	public private(set) var b: CGPoint
...
public class PenShape: Shape, ShapeWithStrokeState {
	...
	public func hitTest(point: CGPoint) -> Bool {
		let x0 = segments.map({ min($0.a.x, $0.b.x) }).min() ?? 0.0
		let y0 = segments.map({ min($0.a.y, $0.b.y) }).min() ?? 0.0
		let x1 = segments.map({ max($0.a.x, $0.b.x) }).max() ?? 0.0
		let y1 = segments.map({ max($0.a.y, $0.b.y) }).max() ?? 0.0
